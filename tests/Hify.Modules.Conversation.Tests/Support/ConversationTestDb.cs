...
internal static class ConversationTestDb
{
    ...
    {
        // 非 0：软删需写入非 0 的 deleted_at 才能被全局过滤命中。
        public long UtcNowEpochMs => 1_700_000_000_000;
    }

    public static ConversationDbContext NewContext() =>
        new(new DbContextOptionsBuilder<ConversationDbContext>().UseNpgsql(ConnectionString).Options, new FixedClock());
